...
if __name__ == '__main__':
    ...
    if len(feats) > 0:
        ...
        if params['lda_abs_th'] < 0.0:
            lda_res, lda_res_th = dict([(k, 0.0) for k, v in list(feats.items())]), dict([(k, v) for k, v in list(feats.items())])
        else:
            if params['rank_tec'] == 'lda':
                lda_res, lda_res_th = test_lda_r(cls, feats, class_sl, params['n_boots'], params['f_boots'], params['lda_abs_th'], 0.0000000001, params['nlogs'])
            elif params['rank_tec'] == 'svm':
                lda_res, lda_res_th = test_svm(cls, feats, class_sl, params['n_boots'], params['f_boots'], params['lda_abs_th'], 0.0, params['svm_norm'])
            else:
                lda_res, lda_res_th = dict([(k, 0.0) for k, v in list(feats.items())]), dict([(k, v) for k, v in list(feats.items())])
    else:
        print("Number of significantly discriminative features:", len(feats), "(", kw_n_ok, ") before internal wilcoxon")
