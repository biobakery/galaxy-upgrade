...
def remove_missing(data, roc):
    ...
    for i, r in enumerate(data):
        if len([v for v in r if not(v == "" or v.isspace())]) < max_len:
            to_rem.append(i)
    if len(to_rem):
...
def numerical_values(feats, norm):
    ...
    for i in range(len(list(feats.values())[0])):
        if hie:
            mul.append(sum([t for j, t in enumerate(tr[i]) if fk[j].count(".") < 1]))
        else:
            mul.append(sum(tr[i]))
    ...
    for f in ff:
        ...
        for l in range(len(fs)):
            ...
            if n in clades2leaves:
                ...
                clades2leaves[n] = [f]
    for k, v in list(clades2leaves.items()):
    ...
    if TotalMetadataEntriesAndIDInBiomFile == 3:  # If there are 3:  Set up default that the second entry is the class and the third is the subclass
        ...
        if not params['biom_class'] is None and not params['biom_subclass'] is None:  # Check if the User passed a valid class and subclass
            ...
            if params['biom_subclass'] in CommonArea['MetadataNames']:
                ...
                FlagError = True
        if FlagError:  # * If the User passed an invalid class
...
if __name__ == '__main__':
    ...
    if isinstance(params['subclass'], int) and int(params['subclass']) < 1:
        params['subclass'] = None
    if isinstance(params['subject'], int) and int(params['subject']) < 1:
        params['subject'] = None
...
#	if 'subclass' in cls.keys(): cls = group_small_subclasses(cls,params['subcl_min_card'])
    ...
    if params['output_table']:
        with open(params['output_table'], "w") as outf:
            if 'class' in cls:
                outf.write("\t".join(list(["class"]) + list(cls['class'])) + "\n")
            if 'subclass' in cls:
                outf.write("\t".join(list(["subclass"]) + list(cls['subclass'])) + "\n")
            if 'subject' in cls:
                outf.write("\t".join(list(["subject"]) + list(cls['subject'])) + "\n")
            for k, v in list(out['feats'].items()):
                outf.write("\t".join([k] + [str(vv) for vv in v]) + "\n")
