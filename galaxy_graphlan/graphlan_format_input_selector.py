...
###############################################################################
#  Modifications log by George Weingart 2022/01/22                            #
#                                                                             #
#  Added, after  return opt + lines the whole pyphlan.py program as the       #
#  import pyphlan.pyphlan as pyphlan does not seem to wor for the             #
#  so called *code file*                                                      #
###############################################################################
...
def get_cols(data, full_names):
    ...
    clade_names_results = PpaTree(
        fname).get_clade_names(full_names=full_names)

    if clade_names_results[0] is None or len(clade_names_results[0]) < 1:
    ...
    lines = []
    try:
        lines = [(red((rc + v.split()[0]), max_len), '%d' % (i + 1), False)
                 for i, v in enumerate(clade_names_results) if v]
